...
更新日期：2026-09-14。0.1.2已在微信后台确认为体验版，管理员已建立，原名单迁移与关闭迁移窗口均完成；关闭后真实读取正常。指定成员及多设备真机验收仍待完成。全部收尾证据见[统一收尾记录](2026-09-14-closeout.md)。
...
| 小程序前端 | 0.1.2已上传，并于9/14在微信后台回读确认为体验版；未提交正式审核或公开发布 |
| 关闭迁移窗口 | 已完成；两函数均移除旧名单变量并关闭回落，SCF接口回读Active、其他变量保留，关闭后真实微信读取成功 |
| 真机验收 | **待完成** |
...
| 窗口开启时间 | 窗口关闭时间 |
| --- | --- |
| 2026-09-13夜部署时开启 | 2026-09-14 11:12–11:17分两函数关闭并复验；11:21由SCF接口最终回读 |
...
### 9月13日部署后的操作顺序（9月14日已完成上传、引导、迁移与关闭）

以下保留当时前端尚未上传时的步骤供溯源；当前0.1.2体验版及迁移关闭已完成，真实状态以本文件顶部与统一收尾记录为准：

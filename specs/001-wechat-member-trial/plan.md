...
继续在整合分支实现001的FR-027/028，不建立新功能目录。

1. `provider.js`：完整正文允许明确的空字符串，仍拒绝缺字段、超长和截断；检查真实标题字段（包括display_title）的长度。
2. `judge.js`：把视频类型交给分类模型，更新制作意图规则；新增可选evidenceSource（旧结果默认desc），所有确定结果均核对原文出处，标题入选只用于视频且必须有制作表达。只有菜名且无制作／排除依据时保留uncertain。模型失败不切通道、不重试。
3. `runner.js`：保留队列内部原排序，常规轮按本周、低粉、仅今日交错，再截取40个；保存后的candidateIds仍按原进度恢复。其他轮次不改变队列策略。
4. `rules.json`和本功能文档：更新规则版本与证据字段约定，预算数值不变。顺带发布此前已审查的HTTP诊断和部分零结果修复。

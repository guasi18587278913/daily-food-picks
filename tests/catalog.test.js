...
test('a round carries its rising accounts on the first page only, and an older snapshot reports none', async () => {
  ...
  const notes = Array.from({ length: 3 }, (_, i) => ({ noteId: id(20 + i), title: '蒸蛋', boards: ['engage'] }));
  ...
  await store.put('dfp_snapshots', '20260913-0900-abcdefabcdef', { id: '20260913-0900-abcdefabcdef', published: true,
    ...
    boards: { today: 0, week: 0, engage: 3, rising: 1 }, accounts });
  ...
  assert.deepEqual(first.data.boards, { today: 0, week: 0, engage: 3, rising: 1 });
  ...
  assert.deepEqual(legacy.data.boards, { today: 0, week: 0, engage: 0, rising: 0 });
});

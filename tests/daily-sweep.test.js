...
test('with the real provider the sweep resumes across ticks, sends at most ten requests per tick and stops at 100 calls', async () => {
  ...
  // 55 searches plus 16 works (detail + history each); three profile lookups fell on a tick's tenth request and
  // were skipped as TICK_LIMIT without dropping the work, and the round budget refused the seventeenth detail.
  ...
  const rows = await store.list('dfp_candidates', { limit: 100, filters: { roundId: '20260913-0600' } });
  assert.equal(rows.filter(row => row.outcome === 'accepted' && row.authorProfileIssue === 'TICK_LIMIT').length, 3);
...
test('the 06:00 sweep admits only today candidates and publishes them', async () => {
  ...
  const snapshot = await readSnapshot(store, result.snapshotId);
  assert.deepEqual(snapshot.boards, { today: 1, week: 0, engage: 0, rising: 0 });
...
test('a 06:00 pick that also crosses the week threshold keeps both boards for the rest of the day', async () => {
  ...
  assert.deepEqual((await readSnapshot(store, sweep.snapshotId)).boards, { today: 1, week: 1, engage: 0, rising: 0 });
  ...
  assert.deepEqual(snapshot.boards, { today: 1, week: 1, engage: 0, rising: 0 });
});
